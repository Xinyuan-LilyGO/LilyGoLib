...
* This page explains how to use the `LilyGO T-LoRa-Pager` with LilyGoLib.
...
* See the [T-LoRa-Pager hardware reference](./hardware/lilygo-t-lora-pager.md) for hardware specifications, interfaces, GPIO assignments, and I2C addresses.
...
> [!IMPORTANT]
>
> LilyGoLib-ThirdParty contains tested dependency versions, which may not be the latest available. Confirm that the hardware works before updating them. If an update causes a problem, restore the tested version listed under [T-LoRa-Pager third-party libraries](./third_party.md#t-lora-pager-third-party).
...
11. If the upload fails or the USB serial port repeatedly connects and disconnects, [manually enter download mode](#t-lora-pager-enter-download-mode) and try again.
...
* In the factory firmware, the keyboard backlight can be controlled only while text input is active.

<a id="t-lora-pager-enter-download-mode"></a>

### Entering Download Mode on the T-LoRa-Pager
